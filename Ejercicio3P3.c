...
int elegir_destino(const Destino *destinos, int n) {
    ...
    int max_conocido = 0; // para apoyar la Regla 4 si asignas un valor simbólico
    double media = 0.0;
    ...
    if (desconocidos > conocidos) {
        int indices_desconocidos[n];
        int num_desconocidos = 0;
        for (int i = 0; i < n; i++) {
            if (!destinos[i].es_conocido) {
                indices_desconocidos[num_desconocidos] = i;
                num_desconocidos++;
            }
        }
        int indice_aleatorio = indices_desconocidos[rand() % num_desconocidos];
        return indice_aleatorio;
    }
    ...
    media = (conocidos > 0) ? suma_conocidos / conocidos : max_conocido + 1;
    ...
    for (int i = 0; i < n; i++) {
        double costo_destino;
        if (destinos[i].es_conocido) {
            costo_destino = destinos[i].costo;
        } else {
            costo_destino = max_conocido + 1; // Valor representativo para desconocidos
        }

        double distancia = abs(costo_destino - media);

        if (distancia_minima == -1.0 || distancia < distancia_minima) {
            distancia_minima = distancia;
            indice_elegido = i;
        }
    }

    return indice_elegido; // Placeholder: reemplaza por el índice elegido
}
